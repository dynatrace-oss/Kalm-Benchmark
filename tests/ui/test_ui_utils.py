...
from kalm_benchmark.ui.utils import gen_utils
from kalm_benchmark.utils.constants import SessionKeys, LAST_SCAN_OPTION


class TestDatabaseBasedSummaryLoading:
    """Test summary loading from database"""
    
    @patch('kalm_benchmark.ui.utils.gen_utils.get_unified_service')
    ...
    def test_ephemeral_results_handling(self, mock_get_service):
        ...
        with patch.object(gen_utils, 'is_ephemeral_scan_result', return_value=True), \
             patch.object(gen_utils, 'load_scan_result'), \
             patch.object(gen_utils, 'SCANNERS') as mock_scanners, \
             patch.object(gen_utils.evaluation, 'evaluate_scanner'), \
             patch.object(gen_utils.evaluation, 'create_summary') as mock_create:
    ...
    @patch('kalm_benchmark.ui.utils.gen_utils.get_unified_service')
    @patch('kalm_benchmark.ui.utils.gen_utils.is_ephemeral_scan_result')
    def test_scan_timestamp_extraction(self, mock_is_ephemeral, mock_get_service):
        ...
        mock_service.get_scanner_result_files.return_value = [
            {'name': 'test-run', 'id': 'scan-id-123'}
        ]
        mock_db = MagicMock()
        mock_service.db = mock_db
        mock_db.get_scan_runs.return_value = [
            {'id': 'scan-id-123', 'timestamp': '2024-01-01T12:00:00'}
        ]
...
class TestDatabaseIntegration:
    ...
    @patch('kalm_benchmark.ui.utils.gen_utils.get_unified_service')
    @patch('kalm_benchmark.ui.utils.gen_utils.st.session_state', {SessionKeys.LatestScanResult: {}})
    ...
    def test_load_scan_result_from_database(self, mock_get_service):
        ...
        mock_service.get_scanner_result_files.return_value = [
            {'name': 'test-run', 'id': 'scan-123'}
        ]
        
        expected_results = [{'check_id': 'TEST-001', 'severity': 'HIGH'}]
        mock_service.load_scanner_results.return_value = expected_results
        
        with patch('kalm_benchmark.ui.utils.gen_utils.is_ephemeral_scan_result', return_value=False):
    ...
    def test_ephemeral_results_in_file_list(self, mock_get_service):
        ...
        with patch('kalm_benchmark.ui.utils.gen_utils.st.session_state', {SessionKeys.LatestScanResult: {'trivy': ['some-data']}}):    
            files = gen_utils.get_result_files_of_scanner("trivy")
